      ...
      class BuildVApp
        ...
        def call(env)
          ...
          if env[:machine].get_vapp_id.nil?
            ...
            # this isn't a new vApp, changing variable to reflect this.
            currentVApp = cnx.get_vapp(env[:machine].get_vapp_id)

            # FIXME: Add a lot of error handling for each step here !

            if currentVApp
              env[:ui].success("VM #{vmName} added to #{currentVApp[:name]} successfully!")
              @logger.info("VM #{vmName} added to #{currentVApp[:name]} successfully!")
